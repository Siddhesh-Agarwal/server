...
class SupabaseInterface:
    ...
    def read(
        self, table, filters=None, select="*", order=None, limit=None, offset=None
    ):
        """
        ...
        if order:
            for column, direction in order.items():
                query = query.order(column, desc=(direction != "asc"))
    ...
    def recordComment(self, data):
        ...
        data = (
            self.client.table("app_comments")
            .upsert(data, on_conflict="issue_id")
            .execute()
        )
        return data.data
    ...
    def checkIsTicket(self, issue_id):
        ccbpResp = (
            self.client.table("issues").select("*").eq("issue_id", issue_id).execute()
        )
        dmpResp = (
            self.client.table("dmp_tickets")
            .select("*")
            .eq("issue_id", issue_id)
            .execute()
        )
    ...
    def checkUnlisted(self, issue_id):
        unlisted_data = (
            self.client.table("unlisted_tickets")
            .select("*")
            .eq("issue_id", issue_id)
            .execute()
        )
    ...
    def deleteTicket(self, issue_id):
        ...
        data = (
            self.client.table("dmp_tickets").delete().eq("issue_id", issue_id).execute()
        )
        return data.data
    ...
    def addPr(self, prData, issue_id):
        ...
        for pr in prData:
            ...
            self.client.table("connected_prs").insert(data).execute()
        return
    ...
    def add_discord_metrics(self, discord_metrics):
        for metric in discord_metrics:
            data = (
                self.client.table("discord_metrics")
                .select("*")
                .eq("product_name", metric["product_name"])
                .execute()
            )
            if len(data.data) > 0:
                data = (
    ...
    def add_engagement(self, github_id):
        contributor = (
            self.client.table("contributor_engagement")
            .select("*")
            .eq("contributor_github", github_id)
            .execute()
        )
        comment_count = contributor.data[0]["github_comments"]
        data = (
            self.client.table("contributor_engagement")
            .update({"github_comments": comment_count + 1})
            .eq("contributor_github", github_id)
            .execute()
        )
        return data
    ...
    def add_github_metrics(self, github_metrics):
        for metric in github_metrics:
            data = data = (
                self.client.table("github_metrics")
                .select("*")
                .eq("product_name", metric["product_name"])
                .execute()
            )
    ...
    def get_org(self, org_name):
        data = (
            self.client.table("dmp_orgs").select("*").like("name", org_name).execute()
        )
        return data.data
...
class PostgresORM:
        ...
        try:
            ...
            async with self.session() as session:
                ...
                stmt = (
                    update(table_class)
                    .where(
                        table_class.pr_id == new_pr_history.pr_id
                    )  # Match the existing issue by issue_id
                    .values()
    ...
    def add_github_user(self, user: str):
        data = (
            self.client.table("contributors_registration")
