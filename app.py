...
from events import TicketEventHandler, TicketFeedbackHandler
from utils import (
    GithubAdapter,
    dispatch_event,
    AddIssueId,
...
async def comment_cleaner():
    while True:
        ...
        for comment in comments:
            ...
            if utc_now - update_time >= datetime.timedelta(
                minutes=15
            ):  # Fixed: use imported timedelta
                url_components = comment["api_url"].split("/")
                owner = url_components[-5]
                repo = url_components[-4]
                comment_id = comment["comment_id"]
                issue_id = comment["issue_id"]
                comment = await TicketFeedbackHandler().deleteComment(
...
async def addIssues():
    ...
    for ticket in tickets:
        ...
        if ticket["status"] == "closed":
            ...
            await TicketEventHandler().onTicketClose(
                {"issue": await get_url(ticket["api_endpoint_url"])}
            )
...
async def updateGithubStats():
    data = await ServerQueries().read(
        "github_profile_data", filters={"dpg_points": ("gt", 0)}
    )
...
async def isAuthenticated():
    print(
        f"already authenticated at {datetime.datetime.now(datetime.timezone.utc)}"
    )  # Fixed: use datetime.datetime
    return await render_template("success.html"), {
        "Refresh": f"2; url=https://discord.com/channels/{os.environ['DISCORD_SERVER_ID']}"
    }
...
async def authenticate(discord_userdata):
    ...
    redirect_uri = f"{os.environ['HOST']}/register/{discord_userdata}"
    # print(redirect_uri)
    github_auth_url = f"https://github.com/login/oauth/authorize?client_id={os.environ['GITHUB_CLIENT_ID']}&redirect_uri={redirect_uri}&scope=user:email"
...
async def register(discord_userdata):
    ...
    print(
        "🛠️SUCCESFULLY DEFINED FUNCTION TO POST TO SUPABASE", locals(), file=sys.stderr
    )
    ...
    return await render_template("success.html"), {
        "Refresh": f"1; url=https://discord.com/channels/{os.environ['DISCORD_SERVER_ID']}"
    }
    ...
    try:
        ...
        for issue in all_issues:
            ...
            res = {
                ...
                "ticket_points": issue["points"]["points"]
                if issue.get("points")
                else None,
                "mentors": ["Amoghavarsh"],
        ...
        print(
            f"Returning {len(issue_result)} filtered issues out of {len(all_issues)} total issues"
        )
        return issue_result
