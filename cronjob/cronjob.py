...
import sys
import time
from utils import logger, NewGenerateJWT
...
class CronJob:
    ...
    def get_github_jwt(self):
        pem = os.environ["PEM_FILE"]
        client_id = os.environ["CLIENT_ID"]
        try:
            with open(pem, mode="rb") as pem_file:
                ...
                key = AbstractJWKBase.from_dict(json.loads(signing_key))
                jwt = JWT()
                encoded_jwt = jwt.encode(payload, key, alg="RS256")
                pem_file.close()
            return encoded_jwt
    ...
    async def get_installations(self, token_headers: dict):
        async with httpx.AsyncClient() as client:
            ...
            installations_response = await client.get(
                get_installations_url, headers=token_headers
            )
            ...
            if installations_response.status_code == 200:
                ...
                if (
                    installations_response.json().get("message", None)
    ...
    async def get_access_token(self, token_headers: dict, installation_id: str):
        async with httpx.AsyncClient() as client:
            ...
            access_token_response = await client.post(
                url=access_token_url, headers=token_headers
            )
            ...
            if access_token_response.status_code == 201:
                ...
                if (
                    access_token_response.json().get("message", None)
                    == "'Expiration time' claim ('exp') must be a numeric value representing the future time at which the assertion expires"
                ):
    ...
    async def get_repos(self, token: str):
        async with httpx.AsyncClient() as client:
            ...
            repo_response = await client.get(url=list_repo_url, headers=token_headers)
            if repo_response.status_code == 200:
                repo_data = repo_response.json()
                return repo_data.get("repositories", [])

    async def get_issues(
        self, token: str, since: datetime, repo_fullname: str, to_date=None
    ):
        page = 1
        all_issues = []
        while True:
            ...
            async with httpx.AsyncClient() as client:
                issues_response = await client.get(
                    url=get_issue_url, headers=token_headers, params=payload
                )
    ...
    async def get_issue_comments(
        self, issue_comment_url, since: datetime, to_date=None, **kwargs
    ):
        page = 1
        ...
        while True:
            ...
            payload = {"since": since.isoformat(), "direction": "asc"}
            ...
            async with httpx.AsyncClient() as client:
                if token:
                    ...
                    response = await client.get(
                        url=comments_url, headers=token_headers, params=payload
                    )
                else:
    ...
    async def get_pull_requests(self, token: str, repo_fullname, since: datetime):
        ...
        while True:
            ...
            async with httpx.AsyncClient() as client:
                pull_requests_response = await client.get(
                    url=get_pull_requests_url, headers=token_headers, params=payload
                )
    ...
    async def main(self, from_date=None, to_date=None):
        ...
        access_tokens = {
            installation.get("id"): await self.get_access_token(
                jwt_headers, installation.get("id")
            )
        ...
        for installation in installations:
            ...
            for repo in repos:
                ...
                await self.process_cron_issues(
                    issues,
                    all_issue_ids,
                    all_comment_ids,
                    from_date=since,
        ...
        try:
            ...
            for issue in issues:
                try:
                    ...
                    data = {"action": state, "issue": issue}
                    if token is not None:
                        await issue_handler.handle_event(
                            data=data, postgres_client="client", token=token
                        )
                    else:
                        await issue_handler.handle_event(
                            data=data, postgres_client="client"
                        )
                    ...
                    all_comments = await self.get_issue_comments(
                        issue["comments_url"], since=since, to_date=to_date, token=token
                    )
                    time.sleep(1)
                    await self.process_cron_issue_comments(
    ...
    async def process_cron_issue_comments(self, issue, all_comments, all_comment_ids):
        try:
            ...
            for comment in all_comments:
                ...
                is_comment_present = await self.postgres_client.get_data(
                    "id", "ticket_comments", comment["id"]
                )

                if is_comment_present is None or len(is_comment_present) == 0:
                    save_data = await self.postgres_client.add_data(
                        comment_data, "ticket_comments"
                    )
                else:
                    save_data = await self.postgres_client.update_data(
                        comment_data, "id", "ticket_comments"
                    )
                print(save_data)
            return True
        except Exception as e:
            ...
            for pr in pull_requests:
                try:
                    ...
                    await pr_handler.handle_event(
                        data={
                            "action": "closed" if pr["state"] == "close" else "opened",
                            "pull_request": pr,
                        },
    ...
    async def purge_issues_comments(self, issue_ids, comment_ids):
        try:
            all_issue_ids_db = await self.postgres_client.read(
                "issues", select_columns="issue_id"
            )
            all_comment_ids_db = await self.postgres_client.read(
                "ticket_comments", select_columns="comment_id"
            )
            ...
            for issue_id in all_issue_ids_db:
                ...
                if not is_present:
                    await self.postgres_client.delete(
                        "issue_contributors", "issue_id", issue_id
                    )
                    await self.postgres_client.delete(
                        "issue_mentors", "issue_id", issue_id
                    )
                    # Delete Ticket
            ...
            for comment_id in all_comment_ids_db:
                ...
                if not is_comment_present:
                    await self.postgres_client.delete(
                        "ticket_comments", "comment_id", comment_id
                    )
    ...
    async def send_discord_report(
        self,
        original_issue_length,
        new_issues_length,
        original_pr_length,
        new_prs_length,
        original_orgs_length,
        new_orgs_length,
        time_taken,
    ):
        try:
            DISCORD_WEBHOOK_URL = os.environ["CRON_DISCORD_WEBHOOK_URL"]
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 204:
                    ...
                    print(
                        f"Failed to send update to Discord. Status: {response.status_code}, Response: {response.text}"
                    )
