...
from utils import GithubAPI, GenerateJWT, MarkdownHeaders
...
class TicketEventHandler:
    ...
    async def onTicketCreate(self, eventData, **kwargs):
        try:
            ...
            if any(
                label["name"].lower() in ["c4gt community".lower(), "dmp 2024"]
                for label in issue["labels"]
            ):
                if any(
                    label["name"].lower() == "c4gt community"
                    for label in issue["labels"]
                ):
                    ticketType = "ccbp"
                else:
                ...
                # org_array = []
                org = await self.postgres_client.get_data(
                    "name", "community_orgs", organization
                )
                if org is None:
                    await self.postgres_client.add_data(
                        data={"name": organization}, table_name="community_orgs"
                    )
                    org = await self.postgres_client.get_data(
                        "name", "community_orgs", organization
                    )
                ...
                ticket_data = {
                    ...
                    if markdown_contents.get("Complexity")
                    else None,
                    "technology": markdown_contents["Tech Skills Needed"]
                    if markdown_contents.get("Tech Skills Needed")
                    else None,
                    "status": issue["state"],
                    "link": issue["html_url"],
                    "org_id": org[0]["id"],
                ...
                if ticketType == "ccbp":
                    recorded_data = await self.postgres_client.record_created_ticket(
                        data=ticket_data, table_name="issues"
                    )
                    print("recorded issue data ", recorded_data)
                    if token is not None:
                        added_contributor = await self.add_contributor(
                            issue, token=token
                        )
                    else:
                ...
                # if (
                #     TicketFeedbackHandler().evaluateDict(markdown_contents)
                #     and ticketType == "ccbp"
                # ):
    ...
    async def onTicketEdit(self, eventData, **kwargs):
        ...
        if ticketType == "ccbp":
            ...
            if token is not None:
                added_contributor = await self.add_contributor(issue, token=token)
            else:
                added_contributor = await self.add_contributor(issue)
            if added_contributor:
                print("contributors data added")
    ...
    async def onTicketClose(self, eventData, **kwargs):
        try:
            token = kwargs.get("token", None)
            print(token)
            issue_update = {"status": "closed", "issue_id": eventData["id"]}

            print("issue_update in ticket close is ", issue_update)
            issue_details = await self.postgres_client.update_data(
                issue_update, "issue_id", "issues"
            )
            ...
            contributors = (
                await self.postgres_client.get_contributors_from_issue_id(
                    issue[0]["id"]
                )
                if issue
                else None
                ...
                if assignee:
                    ...
                    if issue_contributor:
                        ...
                        if assignee_id != issue_contributor["github_id"]:
                            await self.postgres_client.delete(
                                "issue_contributors", "issue_id", issue[0]["id"]
                            )
                            issue_contributor_id = None
                            # Step 7: Look up assignee in contributors_registration table
                            user = await self.postgres_client.get_data(
                                "github_id", "contributors_registration", assignee_id
                            )

                            if user:
                                # Step 8: Register assignee as contributor
                                issue_contributor_id = (
                                    user[0]["id"] if user[0] else None
                                )
                                contributors_data = {
            ...
            points = await self.postgres_client.get_pointsby_complexity(
                issue[0]["complexity"].lower()
            )
            ...
            try:
                ...
                get_issue_mentor = await self.postgres_client.get_data(
                ...
                if get_issue_mentor:
                    try:
                        ...
                        angel_mentor_id = (
                            get_issue_mentor[0]["id"] if get_issue_mentor[0] else None
                        )
                    except Exception as e:
                ...
                if angel_mentor:
                    ...
                    angel_mentor_details = await self.postgres_client.get_data(
                        "github_url",
                    ...
                    if angel_mentor_details:
                        latest_angel_mentor_id = (
                            angel_mentor_details[0]["id"]
                            if angel_mentor_details[0]
                            else None
                        )
                        # Step 7 & 8: Replace the old angel mentor if it differs
                        if angel_mentor_id != latest_angel_mentor_id:
                            await self.postgres_client.delete(
                                "issue_mentors", "issue_id", issue[0]["id"]
                            )
                            mentor_data = {
                                "issue_id": issue[0]["id"],
            ...
            await self.postgres_client.add_data(point_transaction, "point_transactions")
    ...
    async def get_mentors_urls(self, body):
        ...
        if mentors_match:
            ...
            url_match = re.search(
                r"\[.*?\]\((https://github\.com/.*?)\)", mentors_details
            )
            if url_match:
    ...
    async def add_contributor(self, issue, **kwargs):
        try:
            ...
            get_issue = await self.postgres_client.get_data(
            ...
            user = await self.postgres_client.get_data(
                "github_id", "contributors_registration", contributors_id
            )
            contributors_data = {
            ...
            get_issue_in_contributors = await self.postgres_client.get_data(
                "issue_id", "issue_contributors", get_issue[0]["id"]
            )
            inserted_contributors_data = None
            if get_issue_in_contributors:
                inserted_contributors_data = await self.postgres_client.update_data(
                    contributors_data, "issue_id", "issue_contributors"
                )
            else:
                inserted_contributors_data = await self.postgres_client.add_data(
                    contributors_data, "issue_contributors"
                )
            ...
            angel_mentor_detials = await self.postgres_client.get_data(
                "github_url",
                "contributors_registration",
                f"https://github.com/{angel_mentor}",
            )
            if not angel_mentor_detials:
                ...
                if angel_mentor:
                    ...
                    if angel_mentor_data:
                        ...
                        angel_mentor_detials = await self.postgres_client.get_data(
                            "github_id", "contributors_registration", angel_mentor_id
                        )
            mentor_data = {
                "issue_id": get_issue[0]["id"],
            ...
            get_issue_mentor = await self.postgres_client.get_data(
                "issue_id", "issue_mentors", get_issue[0]["id"]
            )
            # inserted_mentors_data = None
            if get_issue_mentor:
                inserted_mentors_data = await self.postgres_client.update_data(
                    mentor_data, "issue_id", "issue_mentors"
                )
            else:
                inserted_mentors_data = await self.postgres_client.add_data(
                    mentor_data, "issue_mentors"
                )
    ...
    async def add_assignee(self, issue):
        try:
            issue_exist = await self.postgres_client.get_data(
                "issue_id", "issues", issue["id"]
            )
            if issue_exist:
