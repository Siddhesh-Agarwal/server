...
class IssuesHandler(EventHandler):
    ...
    async def handle_event(self, data, postgres_client, **kwargs):
        ...
        try:
            ...
            labels = issue["labels"]
            print(
                f"inside issue handler with issue data as {issue} \n label as {labels}"
            )
            if next(
                (
                    label
                    for label in labels
                    if label["name"].lower()
                    in ["c4gt community", "c4gt bounty", "c4gt coding"]
                ),
                None,
            ):
                ...
                if handler_method:
                    if token is not None:
                        await handler_method(data, token=token)
                    else:
    ...
    async def handle_issue_created(self, data, **kwargs):
        ...
        try:
            ...
            if data.get("issue"):
                ...
                if token is not None:
                    await TicketEventHandler().onTicketCreate(data, token=token)
                else:
    ...
    async def handle_issue_unlabeled(self, data):
        try:
            if data["action"] == "unlabeled":
                issue = data["issue"]
                db_issue = await self.postgres_client.get_issue_from_issue_id(
                    issue["id"]
                )
                ...
                if db_issue:
                    ...
                    await self.postgres_client.delete(
                        "issue_contributors", "issue_id", db_issue[0]["id"]
                    )
                    await self.postgres_client.delete(
                        "issue_mentors", "issue_id", db_issue[0]["id"]
                    )
                    # Delete Ticket
    ...
    async def handle_issue_edited(self, data, **kwargs):
        try:
            ...
            db_issue = await self.postgres_client.get_data(
                "issue_id", "issues", issue["id"], "*"
            )
            if not db_issue:
                if token is not None:
                    await self.handle_issue_opened(
                        data, self.postgres_client, token=token
                    )
                else:
            ...
            if body:
                if token is not None:
                    await TicketEventHandler().onTicketEdit(data, token=token)
                else:
    ...
    async def handle_issue_closed(self, data, **kwargs):
        try:
            ...
            print(issue_exist)
            if not issue_exist:
                await self.handle_issue_created(data, token=token)
            issue_exist = await self.postgres_client.get_data(
                "issue_id", "issues", issue["id"], "*"
            )
            print(issue_exist)
            if issue_exist:
                if token is not None:
                    await TicketEventHandler().onTicketClose(issue, token=token)
                else:
