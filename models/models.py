...
class GithubInstallations(Base):
    ...
    created_at = Column(DateTime, nullable=True)
    organisation = Column(
        Text, ForeignKey("community_organisations.name"), nullable=True
    )
...
class MentorshipProgramSiteStructure(Base):
    ...
    project_id = Column(
        BigInteger, ForeignKey("mentorship_program_projects.id"), nullable=True
    )
    contributor_id = Column(
        BigInteger,
        ForeignKey("mentorship_program_selected_contributors.id"),
        nullable=True,
    )
